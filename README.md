...
*   **Dimensionless Analysis:** Determines the Predictive Separation Index (η) using the governing UDCF equation.
...
The rig operates on the principle of the Predictive Separation Index (η):
...
The critical velocity \(v_c\), which represents the minimum velocity required to achieve material severance (where η = 1), incorporates the recovery factor \(k_Y\) and is calculated as:
...
4. Adjust the "Rig Setup" and "Specimen & Recovery Properties" to observe real-time changes in (η).
